...
def adjust_ground_truth(ground_truth, gamma, depth_view):
    return np.asarray(
        [
            round(
                ground_truth[node] * gamma ** (depth - 1),
                3,
            )
            if depth != 0
            else 0
...
def adjust_state(state, gamma, depth_view, include_last_action=False):
    ...
    for node, depth in depth_view:
        if depth == 0:
            ...
            vals = [round(val * gamma ** (depth - 1), 3) for val in state[node].vals]
            new_state.append(Categorical(vals, state[node].probs))
        else:
            val = round(
                state[node] * gamma ** (depth - 1),
                3,
            )
...
def get_states_for_trace(
    ...
    """
    Get states, rewards and finished boolean from actions.
...
def get_trace_from_human_row(
    ...
    """
    Transform a human row to a trace.
